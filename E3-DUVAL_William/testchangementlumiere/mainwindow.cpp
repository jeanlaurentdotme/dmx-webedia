 ...
 *    QSqlQuery query;
    ...
    QObject::connect(ui->pushButton_12, SIGNAL(clicked()), this, SLOT(AjouterLumiere()));
...
void MainWindow::CreerScene()
{
    ...
    //ajout de la scène
    if(name != NULL && couleurscene !=NULL && couleurL1 !=NULL) //si toutes les cases on été remplie
    {
        if(VerifHexa(couleurscene) == true && VerifHexa(couleurL1) == true) //verifie que chacune des couleurs rentrer est bien en Hexadecimal pointer
        {
            QSqlQuery query;
            if(query.exec("INSERT INTO Scene(name,couleurscene,L1,L2,L3) VALUES (' " +name+" ','"+couleurscene+"','"+couleurL1+"')"))  //commande sql ajout de scène
            {
                //std::cout << "La scène à bien été ajoutée" << std::endl;
                ui->label_7->setText("            La scène a bien été créée");
...
void MainWindow::ModifScene()
{
    ...
    else  //sinon effectuer toute la fonction de modification
    {
        ...
        if(nameModif != NULL && couleursceneModif !=NULL && couleurL1Modif !=NULL) //si toutes les cases ont été remplie
        {
            if(VerifHexa(couleursceneModif) == true && VerifHexa(couleurL1Modif) == true)
            {
                ...
                QSqlQuery query;
                if(query.exec("UPDATE Scene SET name = '"+nameModif+"', couleurscene = '"+couleursceneModif+"', L1 = '"+couleurL1Modif+"' WHERE id = "+idModif))
                {
                    //std::cout << "La scène à bien été modifiée" << std::endl;
                    ui->label_16->setText("      La scène a bien été modifiée");
...
void MainWindow::SupprimerScene()
{
    ...
    {
        //std::cout << "La scène a bien été supprimée" << std::endl;
...
//fonction creer un line edit-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void MainWindow::creerLineEdit(int longueur, int hauteur, int x, int y)
{
    QLineEdit *lineEdit = new QLineEdit(this);
    lineEdit->setFixedSize(longueur, hauteur);
    this->move(x, y);
}

//fonction ajouter une lumiere dans creation scene---------------------------------------------------------------------------------------------------------------------------------------------------
void MainWindow::AjouterLumiere()
{
    x = x + 170;
    xajout = xajout + 170;
    creerLineEdit(151,25, x, y);
    ui->pushButton_12->setGeometry(xajout,y,151,25);
}
