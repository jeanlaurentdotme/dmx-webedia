    ...
    ui(new Ui::MainWindow)
{
    ...
    QObject::connect(console, &ConsoleMaterielle::buttonPushed, this, &MainWindow::onButtonStateChanged);
    console->setScene("Test Raspberry");
...
void MainWindow::AfficherListeScene()
{
    ...
    {
        liste.append(QList<QString>());
        for(int j = 0; j < model->record(i).count(); j++)
        {
            liste.back().append(model->record(i).value(j).toString());
        }
    }

    console->setScene(liste[currentSceneNb][1]);
...
void MainWindow::onButtonStateChanged(int buttonId, int value)
{
    qDebug() << "Button " << buttonId << " etat " << value;

    // Left
    if(buttonId == 1 && value == 1)
    {
        currentSceneNb--;
    }
    else if(buttonId == 2 && value == 1)
    {
        currentSceneNb++;
    }
    ...
    if(currentSceneNb < 0)
    {
        currentSceneNb = liste.size() - 1;
    }
    else if(currentSceneNb >= liste.size())
    {
        currentSceneNb = 0;
    }
